...
namespace psu_generic_parser
{
    public class BlewFish
    {
        ...
        public UInt32[] decrypt(UInt32 initialLeft, UInt32 initialRight)
        {
            UInt32 left = initialLeft;
            UInt32 right = initialRight;
            ...
            left ^= P[1];
            right ^= P[0];
            UInt32[] toReturn = { right, left };
            return toReturn;
        }

        public byte[] decryptBlock(byte[] block)
        {
            return decryptBlock(block, block.Length);
        }

        public byte[] decryptBlock(byte[] block, int decryptSize)
        {
            byte[] toReturn = new byte[block.Length];
            int size = block.Length;
            int i = 0;
            for (; (i + 7) < decryptSize; i += 8)
            {
                uint left = BitConverter.ToUInt32(block, i);
                uint right = BitConverter.ToUInt32(block, i + 4);
                if(BigEndian)
                {
                    left = ReverseBytes(left);
                    right = ReverseBytes(right);
                }
                    
                uint[] temp = decrypt(left, right);
                if (BigEndian)
                {
                    temp[0] = ReverseBytes(temp[0]);
                    temp[1] = ReverseBytes(temp[1]);
                }
        ...
        public BlewFish(UInt32 key, bool bigEndian)
        {
            BigEndian = bigEndian;
            setKey(key);
        }
        ...
        public void setKey(UInt32 key)
        {
            ...
            UInt32 temp = BigEndian ? ReverseBytes(key) : key;
            for (int i = 0; i < 18; i++)
                P[i] = bf_pbox[i] ^ temp;
