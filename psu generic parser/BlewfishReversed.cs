...
namespace zamboni
{
    class BlewfishReversed
    {
        ...
        public byte[] decryptBlock(byte[] block)
        {
            ...
            for (; (i + 7) < size; i += 8)
            {
                uint left = ReverseBytes(BitConverter.ToUInt32(block, i));
                uint right = ReverseBytes(BitConverter.ToUInt32(block, i + 4));
                uint[] temp = decrypt(left, right);
                Array.Copy(BitConverter.GetBytes(ReverseBytes(temp[0])), 0, toReturn, i, 4); //BitConverter.GetBytes(temp[0]).CopyTo(decryptedFiles, i);
                Array.Copy(BitConverter.GetBytes(ReverseBytes(temp[1])), 0, toReturn, i + 4, 4);
            }
            if (i < size)
