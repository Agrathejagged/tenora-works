...
namespace psu_generic_parser
{
    public partial class XvrViewer : UserControl
    {
        ...
        public XvrViewer(TextureFile toLoad) : this()
        {
            ...
            {
                label3.Text = (toLoad as XvrTextureFile).currFormat.ToString();
                label5.Text = (toLoad as XvrTextureFile).headerData.ToString("X4");
            }
            if (toLoad is NewXvrTextureFile)
            {
                label3.Text = (toLoad as NewXvrTextureFile).OriginalPixelFormat.ToString();
                label5.Text = (toLoad as NewXvrTextureFile).OriginalTextureType.ToString();
                comboBox1.DataSource = permittedFormats;
                comboBox1.SelectedItem = (toLoad as NewXvrTextureFile).SavePixelFormat;
            }
        ...
        {
            if(internalTexture is NewXvrTextureFile)
            {
                PsuTexturePixelFormat fmt;
                Enum.TryParse<PsuTexturePixelFormat>(comboBox1.SelectedValue.ToString(), out fmt);
                ((NewXvrTextureFile)internalTexture).SavePixelFormat = fmt;
                refreshMip();
            }
        }
    }
